 ...
 {  userInfo &&
    <Container maxWidth='xl' >
        ...
        <Box flex={{xs:'0',sm:'0',md:'4'}} >
          <Typography fontSize={'35px'} fontFamily='Roboto' textAlign={{xs:'center',md:'justify'}}>Shipping Info</Typography>
       <Box display='flex' flexDirection={'column'} justifyContent='center' gap={{xs:'0.7em',md:'1em'}} paddingLeft={{xs:'0',sm:'2em'}} paddingTop='1.5em'>
          <Box display={'flex'} gap={{xs:'0.3em',md:'1em'}} flexDirection={{xs:'column',sm:'row'}} textAlign={{xs:'center',sm:'unset'}}>
            ...
            <Typography color='#757575' fontFamily={'Roboto'}> {userInfo.user.name}</Typography>
          </Box>
          <Box display={'flex'} flexDirection={{xs:'column',sm:'row'}} gap={{xs:'0.3em',md:'1em'}} textAlign={{xs:'center',sm:'unset'}}>
            <Typography>Phone:</Typography>
            <Typography color='#757575' fontFamily={'Roboto'}> {shippingInfo.phoneno}</Typography>
          </Box>
          <Box display={'flex'} flexDirection={{xs:'column',sm:'row'}} gap={{xs:'0.3em',md:'1em'}} textAlign={{xs:'center',sm:'unset'}} >
       ...
       {address}
          ...
          <Box>
          <Typography fontSize={'35px'} fontFamily='Roboto' marginTop={'1.3em'} textAlign={{xs:'center',sm:'unset'}}>Your Cart Items:</Typography>
          <Box paddingLeft={{xs:'0',sm:'0.5em'}} paddingTop='1em' >
            {cartItem && cartItem.map((item,i)=>(
               ...
               <Link to={`/product/${item.productDetails.product._id}`} style={{textDecoration:'none',}}> <Typography fontFamily={'Roboto'} color='#757575' fontSize={{xs:'17px',sm:'22px'}} sx={{'&:hover':{color:'#424242'}}}>{item.productDetails.product.name}</Typography> </Link>
                </Box>
                <Box  display='flex' paddingRight={{xs:'0',sm:'10px'}}>
                  <Typography fontFamily={'Roboto'} color='#757575' fontSize={{xs:'15px',sm:'22px'}} >{item.quantity} X ${item.productDetails.product.price} </Typography>
                  <Typography fontFamily={'Roboto'} fontWeight='bold' fontSize={{xs:'15px',sm:'22px'}} >=${item.quantity * item.productDetails.product.price}</Typography>
                  </Box>
               </Box>
            ))}
          </Box>
        ...
        <Box flex={{xs:'0',sm:'0',md:'2'}}borderLeft={{xs:'none',sm:'none',md:'1px solid #e0e0e0'}} display='flex' flexDirection={'column'} alignItems='center' paddingTop={'2em'}>
         <Box borderBottom={'1px solid black'} width='80%' textAlign={'center'}>
           <Typography  fontSize={'35px'} paddingBottom='0.5em' fontFamily='Roboto' borderBottom={'1px solid #757575'}>Order Summaray</Typography>
           </Box>
           <Box  width={{xs:'90%',sm:'80%'}} display={'flex'} flexDirection='column' gap='35px' paddingTop='20px'>
          <Box display='flex' justifyContent={'space-between'}>
            <Typography fontFamily={'Roboto'} color='#424242'>Subtotal:</Typography>
            <Typography fontFamily={'Roboto'} color='#757575'>${subtotal}</Typography>
          </Box>
          <Box display='flex' justifyContent={'space-between'}>
            <Typography fontFamily={'Roboto'} color='#424242'>Shipping Carges:</Typography>
            <Typography fontFamily={'Roboto'} color='#757575'>${shippingCharges}</Typography>
          </Box>
          <Box display='flex' justifyContent={'space-between'} paddingBottom='20px' borderBottom={'1px solid black'}>
            <Typography fontFamily={'Roboto'} color='#424242'>GST:</Typography>
            <Typography fontFamily={'Roboto'} color='#757575' >${tax}</Typography>
          </Box>
          <Box display='flex' justifyContent={'space-between'}>
            <Typography fontFamily={'Roboto'} fontWeight='bold'>Total:</Typography>
            <Typography fontFamily={'Roboto'} fontWeight='bold' color='#757575'>${totalPrice}</Typography>
          </Box>
