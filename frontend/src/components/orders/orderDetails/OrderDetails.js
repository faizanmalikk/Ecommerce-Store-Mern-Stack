...
{ isFetching ? <Loader/> :(
   ...
   <Container maxWidth='xl'>
        ...
        <Typography fontSize={{sx:'28px',sm:'35px'}} color='#757575' fontFamily='cursive' textAlign={{xs:'center',sm:'justify'}} maxWidth='100%' textOverflow='ellipsis' whiteSpace={'nowrap'} overflow={'hidden'}>Order # {data.order._id}</Typography>  
           </Box>
        <Box>
          <Typography fontSize={'35px'} fontFamily='cursive' textAlign={{xs:'center',sm:'justify'}}>Shipping Info</Typography>
       <Box display='flex' flexDirection={'column'} justifyContent='center' gap={{xs:'0.7em',md:'1em'}} paddingLeft={{xs:'0',sm:'2em'}} paddingTop='0.7em'>
          <Box display={'flex'} gap={{xs:'0.3em',md:'1em'}} flexDirection={{xs:'column',sm:'row'}} textAlign={{xs:'center',sm:'unset'}}>
            ...
            <Typography color='#757575' fontFamily={'cursive'}> {data.order.user && data.order.user.name}</Typography>
          </Box>
          <Box display={'flex'} flexDirection={{xs:'column',sm:'row'}} gap={{xs:'0.3em',md:'1em'}} textAlign={{xs:'center',sm:'unset'}}>
            <Typography>Phone:</Typography>
            <Typography color='#757575' fontFamily={'cursive'}> {data.order.shippingInfo && data.order.shippingInfo.phoneno}</Typography>
          </Box>
          <Box display={'flex'} flexDirection={{xs:'column',sm:'row'}}   gap={{xs:'0.3em',md:'1em'}} textAlign={{xs:'center',sm:'unset'}} >
  ...
  <Typography textAlign={{xs:'center',sm:'justify'}} textTransform='uppercase' paddingLeft={{xs:'0',sm:'2em'}} paddingTop='0.7em' color = {`${data.order.orderStatus === 'delivered' ? 'green' : 'red'}`}>
    {data.order.orderStatus}
        ...
        <Typography  fontSize={'35px'} fontFamily='cursive' paddingBottom={'1.3em'} textAlign={{xs:'center',sm:'justify'}}>Order Items:</Typography>
